...
SET (LIBRARY_OUTPUT_PATH ${VTKAG_BINARY_DIR}/bin/ CACHE PATH 
     "Single output directory for building all libraries.")

SET (EXECUTABLE_OUTPUT_PATH ${VTKAG_BINARY_DIR}/bin/ CACHE PATH 
...
IF (USE_VTK_FILE)
  ...
  SET(VTKAG_BUILD_SHARED_LIBS ${BUILD_SHARED_LIBS} CACHE INTERNAL 
      "Is this VTKAG built with shared libraries.")
...
IF (VTK_WRAP_TCL)
  ...
  IF (VTKAG_WRAP_TCL)
    ...
    ELSE (NOT VTK_WRAP_TCL_EXE)
      ...
      FIND_FILE (VTK_WRAP_HINTS hints ${VTKAG_SOURCE_DIR}/Wrapping)
  ...
  ENDIF (VTKAG_WRAP_TCL)

ELSE (VTK_WRAP_TCL)

  IF (VTKAG_WRAP_TCL)
    MESSAGE("Warning. VTKAG_WRAP_TCL is ON but the VTK version you have chosen has not support for Tcl (VTK_WRAP_TCL is OFF). Please set VTKAG_WRAP_TCL to OFF.")
    SET (VTKAG_WRAP_TCL OFF)
  ENDIF (VTKAG_WRAP_TCL)
...
IF (VTK_WRAP_PYTHON)
  ...
  ENDIF (VTKAG_WRAP_PYTHON)

ELSE (VTK_WRAP_PYTHON)

  IF (VTKAG_WRAP_PYTHON)
    MESSAGE("Warning. VTKAG_WRAP_PYTHON is ON but the VTK version you have chosen has not support for Python (VTK_WRAP_PYTHON is OFF). Please set VTKAG_WRAP_PYTHON to OFF.")
    SET (VTKAG_WRAP_PYTHON OFF)
  ENDIF (VTKAG_WRAP_PYTHON)
...
IF (VTK_WRAP_JAVA)
  ...
  IF (VTKAG_WRAP_JAVA)
    ...
    IF (VTK_WRAP_JAVA_EXE AND VTK_PARSE_JAVA_EXE)
      ...
      IF (NOT VTK_JAVA_HOME)
        SET (VTK_JAVA_HOME ${VTKAG_BINARY_DIR}/java/vtk CACHE PATH "Path to Java install")
  ...
  ENDIF (VTKAG_WRAP_JAVA)

ELSE (VTK_WRAP_JAVA)

  IF (VTKAG_WRAP_JAVA)
    MESSAGE("Warning. VTKAG_WRAP_JAVA is ON but the VTK version you have chosen has not support for Java (VTK_WRAP_JAVA is OFF). Please set VTKAG_WRAP_JAVA to OFF.")
    SET (VTKAG_WRAP_JAVA OFF)
  ENDIF (VTKAG_WRAP_JAVA)
