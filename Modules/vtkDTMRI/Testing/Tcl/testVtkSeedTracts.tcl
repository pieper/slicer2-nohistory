...
set streamlines [seedTracts GetStreamlines]
$streamlines InitTraversal
foreach s "1 2 3 4 5" {
    
    set stream [$streamlines GetNextItemAsObject]

    vtkPolyDataMapper mapper$s
    mapper$s SetInput [$stream GetOutput]
    mapper$s SetLookupTable lut

    vtkActor actor$s
    actor$s SetMapper mapper$s 
    
    ren1 AddActor actor$s
}
