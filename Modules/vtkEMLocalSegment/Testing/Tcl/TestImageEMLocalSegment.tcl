...
puts "Result will be written as TestImageEMLocalSegmentTestResult.* - compare it to TestImageEMLocalSegmentResult.*"
...
puts "1a= Just the E-Step with MulitThreading activated  (alpha = 0 and iter = 1) (same result as 1)"
puts "2 = E-Step with MFA               (alpha = 0.9, iter = 1, and MFAIter = 1)"
puts "2a= E-Step with MFA  with MulitThreading activated (alpha = 0.9, iter = 1, and MFAIter = 1) (same result as 2)"
puts "3 = E-Step and M-Step without MFA (alpha = 0.0, iter = 2, MFAIter = 0)" 
...
puts "9 = Joint Registration and Segmentation (images are rotated by -3 deg in the Z axis - results do not make sense (bc only 3 slices)"
puts "Number of Evaluations :2 Global Parameters: 3.89896 3.5482 4.30863 3.76829 0.612417 2.12405 28.3986 31.016 31.016 WeightAtlas Cost: 43344.8"
...
proc EMSegmentSetVtkSuperClassSetting {SuperClass} {
  ...
  if {$EMSegment(SegmentMode)} {
      EMSegment(Cattrib,$SuperClass,vtkImageEMSuperClass) SetStopEMMaxIter $EMSegment(EMiteration)  
      EMSegment(Cattrib,$SuperClass,vtkImageEMSuperClass) SetStopMFAMaxIter $EMSegment(MFAiteration)  
  }
...
proc EMSegmentStartEM {} {
   ...
   } else {vtkImageEMLocalSegmenter EMSegment(vtkEMSegment) }
   ...
   catch {EMSegment(vtkEMSegment) SetDisableMultiThreading $EMSegment(DisableMultiThreading)}
   ...
   if {$EMSegment(SegmentMode) > 1} {
       ...
       # puts "--------------------"
       # EMSegment(Cattrib,0,vtkImageEMLocalSuperClass) Update
       # puts "--------------------"
       # puts [EMSegment(Cattrib,0,vtkImageEMLocalSuperClass) Print]
   } else {
...
proc CreateSuperClass {SuperClass NumClasses StartIndex} {
  ...
  for {set i $StartIndex} { $i < $max} {incr i} {
      ...
      set EMSegment(Cattrib,$i,RegistrationType) 0 
      set EMSegment(Cattrib,$i,GenerateBackgroundProbability) 0
      set EMSegment(Cattrib,$i,RegistrationIndependentSubClassFlag) 0
  }
}
...
proc LoadFile {fileName Mode} {
    ...
    foreach pair $tags {
      ...
      set EMSegment(PrintDir) "."
          ...
          # Make sure it runs the same on all machines 
          set EMSegment(DisableMultiThreading) 1
      ...
      set EMSegment(Cattrib,0,PrintPCA)  0
      # SuperClass Variables
      set EMSegment(Cattrib,0,PrintBias) 0
      set EMSegment(Cattrib,0,PrintLabelMap) 0 
      set EMSegment(Cattrib,0,PrintFrequency) 0

          set EMSegment(Cattrib,0,RegistrationType) 0 
          set EMSegment(Cattrib,0,GenerateBackgroundProbability) 0
          set EMSegment(Cattrib,0,RegistrationIndependentSubClassFlag) 0
   ...
   switch $Mode {
       ...
       9 {  set EMSegment(Alpha) 0
            ...
            set EMSegment(Cattrib,0,RegistrationType) 2
         }
   }
   return 1
}
...
#-------------------------------------------------------------------------------
switch $Mode {
    "8" { set XMLFile  "$env(PWD)/Modules/vtkEMLocalSegment/Testing/Tcl/TestImageEMLocalSegmentDataHir.xml"}
    default {set XMLFile "$env(PWD)/Modules/vtkEMLocalSegment/Testing/Tcl/TestImageEMLocalSegmentData.xml"}
}
...
if {[LoadFile $XMLFile $Mode] < 1} {
  puts "Error:: Could not load the file $XMLFile"
  exit 1 
}
...
vtkImageReader Volume(1,vol)
  ...
  if {$Mode == "1a"} { set Mode 1}
  if {$Mode == "2a"} { set Mode 2}

  Volume(1,vol) SetFilePrefix  $env(PWD)/Modules/vtkEMLocalSegment/Testing/Tcl/TestImageEMLocalSegmentMode$Mode
