...
ELSE (USE_VTK_FILE)
  SET (VTKFASTMARCHING_CAN_BUILD 0)
ENDIF (USE_VTK_FILE)
...
SET (LIBRARY_OUTPUT_PATH ${VTKFASTMARCHING_BINARY_DIR}/bin/ CACHE PATH 
     "Single output directory for building all libraries.")

SET (EXECUTABLE_OUTPUT_PATH ${VTKFASTMARCHING_BINARY_DIR}/bin/ CACHE PATH 
...
IF (USE_VTK_FILE)
  ...
  SET(VTKFASTMARCHING_BUILD_SHARED_LIBS ${BUILD_SHARED_LIBS} CACHE INTERNAL 
      "Is this VTKFASTMARCHING built with shared libraries.")
...
IF (VTK_WRAP_TCL)
  ...
  OPTION(VTKFASTMARCHING_WRAP_TCL 
         "Wrap classes into the TCL interpreted language." 
         ON)

  IF (VTKFASTMARCHING_WRAP_TCL)
    ...
    ELSE (NOT VTK_WRAP_TCL_EXE)
      ...
      FIND_FILE (VTK_WRAP_HINTS hints ${VTKFASTMARCHING_SOURCE_DIR}/Wrapping)
  ...
  ENDIF (VTKFASTMARCHING_WRAP_TCL)

ELSE (VTK_WRAP_TCL)

  IF (VTKFASTMARCHING_WRAP_TCL)
    MESSAGE("Warning. VTKFASTMARCHING_WRAP_TCL is ON but the VTK version you have chosen has not support for Tcl (VTK_WRAP_TCL is OFF). Please set VTKFASTMARCHING_WRAP_TCL to OFF.")
    SET (VTKFASTMARCHING_WRAP_TCL OFF)
  ENDIF (VTKFASTMARCHING_WRAP_TCL)
...
IF (VTK_WRAP_PYTHON)
  ...
  OPTION(VTKFASTMARCHING_WRAP_PYTHON 
         "Wrap classes into the Python interpreted language." 
         ON)

  IF (VTKFASTMARCHING_WRAP_PYTHON)
  ...
  ENDIF (VTKFASTMARCHING_WRAP_PYTHON)

ELSE (VTK_WRAP_PYTHON)

  IF (VTKFASTMARCHING_WRAP_PYTHON)
    MESSAGE("Warning. VTKFASTMARCHING_WRAP_PYTHON is ON but the VTK version you have chosen has not support for Python (VTK_WRAP_PYTHON is OFF). Please set VTKFASTMARCHING_WRAP_PYTHON to OFF.")
    SET (VTKFASTMARCHING_WRAP_PYTHON OFF)
  ENDIF (VTKFASTMARCHING_WRAP_PYTHON)
...
IF (VTK_WRAP_JAVA)
  ...
  OPTION(VTKFASTMARCHING_WRAP_JAVA 
         "Wrap classes into the Java interpreted language." 
         ON)

  IF (VTKFASTMARCHING_WRAP_JAVA)
    ...
    IF (VTK_WRAP_JAVA_EXE AND VTK_PARSE_JAVA_EXE)
      ...
      IF (NOT VTK_JAVA_HOME)
        SET (VTK_JAVA_HOME ${VTKFASTMARCHING_BINARY_DIR}/java/vtk CACHE PATH "Path to Java install")
  ...
  ENDIF (VTKFASTMARCHING_WRAP_JAVA)

ELSE (VTK_WRAP_JAVA)

  IF (VTKFASTMARCHING_WRAP_JAVA)
    MESSAGE("Warning. VTKFASTMARCHING_WRAP_JAVA is ON but the VTK version you have chosen has not support for Java (VTK_WRAP_JAVA is OFF). Please set VTKFASTMARCHING_WRAP_JAVA to OFF.")
    SET (VTKFASTMARCHING_WRAP_JAVA OFF)
  ENDIF (VTKFASTMARCHING_WRAP_JAVA)
