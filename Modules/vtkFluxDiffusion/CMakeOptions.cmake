...
ELSE (USE_VTK_FILE)
  SET (VTKFLUXDIFFUSION_CAN_BUILD 0)
ENDIF (USE_VTK_FILE)
...
SET (LIBRARY_OUTPUT_PATH ${VTKFLUXDIFFUSION_BINARY_DIR}/bin/ CACHE PATH 
     "Single output directory for building all libraries.")

SET (EXECUTABLE_OUTPUT_PATH ${VTKFLUXDIFFUSION_BINARY_DIR}/bin/ CACHE PATH 
...
IF (USE_VTK_FILE)
  ...
  SET(VTKFLUXDIFFUSION_BUILD_SHARED_LIBS ${BUILD_SHARED_LIBS} CACHE INTERNAL 
      "Is this VTKFLUXDIFFUSION built with shared libraries.")
...
IF (VTK_WRAP_TCL)
  ...
  OPTION(VTKFLUXDIFFUSION_WRAP_TCL 
         "Wrap classes into the TCL interpreted language." 
         ON)

  IF (VTKFLUXDIFFUSION_WRAP_TCL)
    ...
    ELSE (NOT VTK_WRAP_TCL_EXE)
      ...
      FIND_FILE (VTK_WRAP_HINTS hints ${VTKFLUXDIFFUSION_SOURCE_DIR}/Wrapping)
  ...
  ENDIF (VTKFLUXDIFFUSION_WRAP_TCL)

ELSE (VTK_WRAP_TCL)

  IF (VTKFLUXDIFFUSION_WRAP_TCL)
    MESSAGE("Warning. VTKFLUXDIFFUSION_WRAP_TCL is ON but the VTK version you have chosen has not support for Tcl (VTK_WRAP_TCL is OFF). Please set VTKFLUXDIFFUSION_WRAP_TCL to OFF.")
    SET (VTKFLUXDIFFUSION_WRAP_TCL OFF)
  ENDIF (VTKFLUXDIFFUSION_WRAP_TCL)
...
IF (VTK_WRAP_PYTHON)
  ...
  OPTION(VTKFLUXDIFFUSION_WRAP_PYTHON 
         "Wrap classes into the Python interpreted language." 
         ON)

  IF (VTKFLUXDIFFUSION_WRAP_PYTHON)
  ...
  ENDIF (VTKFLUXDIFFUSION_WRAP_PYTHON)

ELSE (VTK_WRAP_PYTHON)

  IF (VTKFLUXDIFFUSION_WRAP_PYTHON)
    MESSAGE("Warning. VTKFLUXDIFFUSION_WRAP_PYTHON is ON but the VTK version you have chosen has not support for Python (VTK_WRAP_PYTHON is OFF). Please set VTKFLUXDIFFUSION_WRAP_PYTHON to OFF.")
    SET (VTKFLUXDIFFUSION_WRAP_PYTHON OFF)
  ENDIF (VTKFLUXDIFFUSION_WRAP_PYTHON)
...
IF (VTK_WRAP_JAVA)
  ...
  OPTION(VTKFLUXDIFFUSION_WRAP_JAVA 
         "Wrap classes into the Java interpreted language." 
         ON)

  IF (VTKFLUXDIFFUSION_WRAP_JAVA)
    ...
    IF (VTK_WRAP_JAVA_EXE AND VTK_PARSE_JAVA_EXE)
      ...
      IF (NOT VTK_JAVA_HOME)
        SET (VTK_JAVA_HOME ${VTKFLUXDIFFUSION_BINARY_DIR}/java/vtk CACHE PATH "Path to Java install")
  ...
  ENDIF (VTKFLUXDIFFUSION_WRAP_JAVA)

ELSE (VTK_WRAP_JAVA)

  IF (VTKFLUXDIFFUSION_WRAP_JAVA)
    MESSAGE("Warning. VTKFLUXDIFFUSION_WRAP_JAVA is ON but the VTK version you have chosen has not support for Java (VTK_WRAP_JAVA is OFF). Please set VTKFLUXDIFFUSION_WRAP_JAVA to OFF.")
    SET (VTKFLUXDIFFUSION_WRAP_JAVA OFF)
  ENDIF (VTKFLUXDIFFUSION_WRAP_JAVA)
