...
ELSE (USE_VTK_FILE)
  SET (VTKREALIGNRESAMPLE_CAN_BUILD 0)
ENDIF (USE_VTK_FILE)
...
SET (LIBRARY_OUTPUT_PATH ${VTKREALIGNRESAMPLE_BINARY_DIR}/bin/ CACHE PATH 
     "Single output directory for building all libraries.")

SET (EXECUTABLE_OUTPUT_PATH ${VTKREALIGNRESAMPLE_BINARY_DIR}/bin/ CACHE PATH 
...
IF (USE_VTK_FILE)
  ...
  # vtkRealignResampleConfigure.h.in otherwise the BUILD_SHARED_LIB
  # from VTK's vtkConfigure.h file is picked first :(

  SET(VTKREALIGNRESAMPLE_BUILD_SHARED_LIBS ${BUILD_SHARED_LIBS} CACHE INTERNAL 
      "Is this VTKREALIGNRESAMPLE built with shared libraries.")
...
IF (VTK_WRAP_TCL)
  ...
  OPTION(VTKREALIGNRESAMPLE_WRAP_TCL 
         "Wrap classes into the TCL interpreted language." 
         ON)

  IF (VTKREALIGNRESAMPLE_WRAP_TCL)
    ...
    ELSE (NOT VTK_WRAP_TCL_EXE)
      ...
      FIND_FILE (VTK_WRAP_HINTS hints ${VTKREALIGNRESAMPLE_SOURCE_DIR}/Wrapping)
  ...
  ENDIF (VTKREALIGNRESAMPLE_WRAP_TCL)

ELSE (VTK_WRAP_TCL)

  IF (VTKREALIGNRESAMPLE_WRAP_TCL)
    MESSAGE("Warning. VTKREALIGNRESAMPLE_WRAP_TCL is ON but the VTK version you have chosen has not support for Tcl (VTK_WRAP_TCL is OFF). Please set VTKREALIGNRESAMPLE_WRAP_TCL to OFF.")
    SET (VTKREALIGNRESAMPLE_WRAP_TCL OFF)
  ENDIF (VTKREALIGNRESAMPLE_WRAP_TCL)
...
IF (VTK_WRAP_PYTHON)
  ...
  OPTION(VTKREALIGNRESAMPLE_WRAP_PYTHON 
         "Wrap classes into the Python interpreted language." 
         ON)

  IF (VTKREALIGNRESAMPLE_WRAP_PYTHON)
  ...
  ENDIF (VTKREALIGNRESAMPLE_WRAP_PYTHON)

ELSE (VTK_WRAP_PYTHON)

  IF (VTKREALIGNRESAMPLE_WRAP_PYTHON)
    MESSAGE("Warning. VTKREALIGNRESAMPLE_WRAP_PYTHON is ON but the VTK version you have chosen has not support for Python (VTK_WRAP_PYTHON is OFF). Please set VTKREALIGNRESAMPLE_WRAP_PYTHON to OFF.")
    SET (VTKREALIGNRESAMPLE_WRAP_PYTHON OFF)
  ENDIF (VTKREALIGNRESAMPLE_WRAP_PYTHON)
...
IF (VTK_WRAP_JAVA)
  ...
  OPTION(VTKREALIGNRESAMPLE_WRAP_JAVA 
         "Wrap classes into the Java interpreted language." 
         ON)

  IF (VTKREALIGNRESAMPLE_WRAP_JAVA)
    ...
    IF (VTK_WRAP_JAVA_EXE AND VTK_PARSE_JAVA_EXE)
      ...
      IF (NOT VTK_JAVA_HOME)
        SET (VTK_JAVA_HOME ${VTKREALIGNRESAMPLE_BINARY_DIR}/java/vtk CACHE PATH "Path to Java install")
  ...
  ENDIF (VTKREALIGNRESAMPLE_WRAP_JAVA)

ELSE (VTK_WRAP_JAVA)

  IF (VTKREALIGNRESAMPLE_WRAP_JAVA)
    MESSAGE("Warning. VTKREALIGNRESAMPLE_WRAP_JAVA is ON but the VTK version you have chosen has not support for Java (VTK_WRAP_JAVA is OFF). Please set VTKREALIGNRESAMPLE_WRAP_JAVA to OFF.")
    SET (VTKREALIGNRESAMPLE_WRAP_JAVA OFF)
  ENDIF (VTKREALIGNRESAMPLE_WRAP_JAVA)
