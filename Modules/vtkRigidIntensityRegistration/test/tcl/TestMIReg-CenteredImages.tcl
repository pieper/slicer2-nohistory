...
if {$standalone > 0} {
  ...
  if {[llength $argv ] != 7} {
    ...
    puts "This version centers the initial image before applying the transform"
    puts "Usage: vtk TestMIReg-CenteredImages.tcl ImageBase"
...
proc TestMutualInformationTransform { BaseFileName Numx Numy Numz SpaceX SpaceY SpaceZ } {
  ...
  vtkImageReader __reader1
    ...
    catch { __sourcechangeinfo1 Delete   }
   vtkImageChangeInformation __sourcechangeinfo1
     __sourcechangeinfo1 CenterImageOn
     __sourcechangeinfo1 SetInput [__reader1 GetOutput]
  
  set inputvol  [__reader1 GetOutput]
  set inputvol  [__sourcechangeinfo1 GetOutput]
  ...
  ### An initial matrix
   ...
   vtkITKMutualInformationTransform __reg
    ...
    if {[__reg GetError] > 0} {
       puts "Registration Algorithm returned an error!\n Are you sure the images overlap?"
       exit
    }
